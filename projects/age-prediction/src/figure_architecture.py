...
plt.rcParams.update({'font.family':['Arial','Helvetica','DejaVu Sans'],'font.size':15,'svg.fonttype':'none','pdf.fonttype':42,'axes.spines.top':False,'axes.spines.right':False})
...
def box(a,x,y,w,h,title,detail,fill='white',edge=None):
 a.add_patch(FancyBboxPatch((x,y),w,h,boxstyle='round,pad=0.014,rounding_size=0.08',facecolor=fill,edgecolor=edge or P['gray'],linewidth=1.4))
...
def label(a,letter,title,subtitle):
 ...
 a.text(.45,3.03,title,fontweight='bold',fontsize=17,color=P['ink'])
